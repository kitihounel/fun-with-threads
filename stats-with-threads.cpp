 ...
 */
void generate_stats(const Country &country, std::map<std::string, float> &stats)
{
...
int main()
{
    ...
    std::vector<std::map<std::string, float>> stats(std::size(countries));
    bool should_animate = true;
    std::thread thread_animate(animate, &should_animate);

    for (size_t i = 0; i < std::size(countries); ++i) {
        std::thread t(generate_stats, std::cref(countries[i]), std::ref(stats[i]));
